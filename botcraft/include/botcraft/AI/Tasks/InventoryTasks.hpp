...
/// @param button_num Button clicked (same as in
/// ServerboundContainerClickPacket)
/// @return Success if the slots is clicked (for versions < 1.17 and the server
/// confirms it), Failure otherwise
...
/// @brief Same thing as ClickSlotInContainer, but reads its parameters from the
/// blackboard
/// @param client The client performing the action
/// @return Success if the slots is clicked (for versions < 1.17 and the server
/// confirms it), Failure otherwise
...
/// @return Success if the two slots have been correctly swapped, Failure
/// otherwise
...
/// @brief Same thing as SwapItemsInContainer, but reads its parameters from the
/// blackboard
/// @param client The client performing the action
/// @return Success if the two slots have been correctly swapped, Failure
/// otherwise
...
Status DropItemsFromContainer(BehaviourClient &client, const short container_id,
                              const short slot_id, const short num_to_keep = 0);

/// @brief Same thing as DropItemsFromContainer, but reads its parameters from
/// the blackboard
/// @param client The client performing the action
/// @return Success if items were correctly dropped, Failure otherwise
...
/// @brief Same thing as PutOneItemInContainerSlot, but reads its parameters
/// from the blackboard
/// @param client The client performing the action
/// @return Success if the item is correctly set, Failure otherwise
...
/// @brief Same thing as SetItemInHand, but reads its parameters from the
/// blackboard
/// @param client The client performing the action
/// @return Success if the item is now in hand, Failure otherwise
...
/// @param wait_confirmation If true, waits for the server to send the new block
/// in pos
/// @param allow_midair_placing If false, task will fail if the neighbour block
/// matching face is air
/// @return Success if placement attempt was made (and confirmed by the server
/// if wait_confirmation is true), Failure otherwise
...
/// @brief Same thing as PlaceBlock, but reads its parameters from the
/// blackboard
/// @param client The client performing the action
/// @return Success if placement attempt was made (and confirmed by the server
/// if wait_confirmation is true), Failure otherwise
...
/// @param wait_confirmation If true, waits for the eaten stack to be reduced by
...
/// @param client The client performing the action
/// @return Success if the item was eaten (and confirmed if wait_confirmation is
...
/// @brief Same thing as OpenContainer, but reads its parameters from the
/// blackboard
/// @param client The client performing the action
/// @return Success if the container is opened, Failure otherwise
...
/// @param client The client performing the action
/// @param container_id The id of the container to close, if -1, will close the
/// first one found
/// @return Always return Success
Status CloseContainer(BehaviourClient &client, const short container_id = -1);

/// @brief Same thing as CloseContainer, but reads its parameters from the
/// blackboard
/// @param client The client performing the action
/// @return Always return Success
...
/// @return Always return Success
Status LogInventoryContent(BehaviourClient &client,
                           const LogLevel level = LogLevel::Info);

/// @brief Same thing as LogInventoryContent, but reads its parameters from the
/// blackboard
/// @param client The client performing the action
/// @return Always return Success
...
Status TradeName(BehaviourClient &client, const std::string &item_name,
                 const bool buy, const int trade_id = -1);
...
/// @brief Put item in a crafting container and click on the output, storing it
/// in the inventory.
/// @param client The client performing the action
/// @param inputs Input items IDs in a 3x3 grid, inputs[0][1] refers to first
/// line, second column
/// @param allow_inventory_craft If true, the client will use the inventory
/// small 2x2 grid to craft if possible
...
/// in the inventory.
/// @param client The client performing the action
/// @param inputs Input items names in a 3x3 grid, inputs[0][1] refers to first
/// line, second column
/// @param allow_inventory_craft If true, the client will use the inventory
/// small 2x2 grid to craft if possible
/// @return Success if item is crafted, Failure otherwise
...
/// @brief Same thing as CraftNamed, but reads its parameters from the
/// blackboard
/// @param client The client performing the action
/// @return Success if item is crafted, Failure otherwise
...
/// @brief Same thing as HasItemInInventory, but reads its parameters from the
/// blackboard
/// @param client The client performing the action
/// @return Success if inventory quantity is >= quantity else Failure
